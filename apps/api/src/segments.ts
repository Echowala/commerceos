...
const json = (res: ServerResponse, status: number, data: unknown) => { res.statusCode = status; res.setHeader("content-type", "application/json; charset=utf-8"); res.end(JSON.stringify(data)); };
const readBody = async (req: IncomingMessage) => { let raw = ""; for await (const chunk of req) raw += chunk; if (raw.length > 100_000) throw new Error("PAYLOAD_TOO_LARGE"); return raw ? JSON.parse(raw) : {}; };
...
const fields = new Set(["orderCount", "totalSpend", "lastOrderDaysAgo", "tag"]);
const operators = new Set(["gte", "lte", "eq", "neq", "gt", "lt"]);
const validRules = (value: unknown): value is SegmentRules => {
  ...
  return input.rules.every(rule => {
    ...
    if (!fields.has(String(item.field)) || !operators.has(String(item.operator))) return false;
    if (item.field === "tag") return item.operator === "eq" || item.operator === "neq" ? typeof item.value === "string" && item.value.trim().length > 0 && item.value.trim().length <= 80 : false;
    const n = Number(item.value);
    return Number.isFinite(n) && n >= 0;
  });
};

const matches = (rules: SegmentRules, customer: { id: string; orders: { total: unknown; status: string; createdAt: Date }[]; tags: { tag: { name: string } }[] }) => {
...
export const handleSegmentsRequest = async (req: IncomingMessage, res: ServerResponse): Promise<boolean> => {
  ...
  try {
    ...
    if (url.pathname === "/crm/segments" && req.method === "GET") {
      ...
      return json(res, 200, segments.map(segment => ({ ...segment, customerCount: validRules(segment.rules) ? customers.filter(customer => matches(segment.rules, customer)).length : 0 })));
    }
    if (url.pathname === "/crm/segments" && req.method === "POST") {
      const input = await readBody(req); const name = String(input.name ?? "").trim(); const description = input.description == null ? null : String(input.description).trim() || null;
    ...
    if (members && req.method === "GET") {
      ...
      if (!validRules(segment.rules)) return json(res, 500, { error: "invalid_segment_rules" });
      ...
      const result = customers.filter(customer => matches(segment.rules, customer)).map(customer => {
        const validOrders = customer.orders.filter(order => order.status !== "CANCELLED" && order.status !== "REFUNDED");
