...
export const handleRequest = async (req: IncomingMessage, res: ServerResponse) => {
  ...
  try {
    ...
    if (orderStatusMatch && req.method === "PATCH") {
      ...
      if (!orderStatuses.includes(status as typeof orderStatuses[number])) return json(res, 400, { error: "invalid_order_status" });
      const order = await prisma.order.findFirst({ where: { id: orderStatusMatch[1], tenantId }, include: { items: true } });
      if (!order) return json(res, 404, { error: "order_not_found" });
      if (order.status === status) return json(res, 200, { id: order.id, orderNumber: order.orderNumber, status: order.status });
      const shouldReleaseStock = stockReleasingStatuses.has(status) && !stockReleasingStatuses.has(order.status);
      const updated = await prisma.$transaction(async tx => {
        if (shouldReleaseStock) {
          const existingReturn = await tx.inventoryMovement.findFirst({ where: { tenantId, referenceId: order.id, type: "RETURN" }, select: { id: true } });
          if (!existingReturn) {
            for (const item of order.items) {
              const variant = await tx.productVariant.findFirst({ where: { id: item.variantId, product: { store: { tenantId } } }, select: { id: true, productId: true, stock: true } });
        ...
        return tx.order.update({ where: { id: order.id }, data: { status: status as typeof order.status } });
      });
      return json(res, 200, { id: updated.id, orderNumber: updated.orderNumber, status: updated.status });
    }
    if (url.pathname === "/orders" && req.method === "POST") {
      ...
      if (!orderStatuses.includes(requestedStatus as typeof orderStatuses[number])) return json(res, 400, { error: "invalid_order_status" });
      if (stockReleasingStatuses.has(requestedStatus)) return json(res, 400, { error: "cannot_create_order_in_releasing_status" });
