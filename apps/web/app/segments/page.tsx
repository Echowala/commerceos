    ...
    <main className="main workspace-page">
      <header className="header">
        <div>
          ...
          <div className="muted">Build live audiences from customer behavior and CRM tags.</div>
        </div>
        <a className="back-link" href="/customers">Customers</a>
      </header>
      ...
      <section className="section">
        ...
        <div className="segment-form">
          ...
          <div className="segment-rule-head">
            <strong>Rules</strong>
          ...
          {rules.map((rule, index) => (
            <div className="segment-rule" key={index}>
              <select value={rule.field} onChange={e => updateRule(index, { field: e.target.value as Rule["field"], value: e.target.value === "tag" ? "" : 1 })}>
                {Object.entries(fieldLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}
              </select>
              <select value={rule.operator} onChange={e => updateRule(index, { operator: e.target.value as Rule["operator"] })}>
                {operators.map(operator => <option key={operator} value={operator}>{operatorLabels[operator]}</option>)}
              </select>
              <input type={rule.field === "tag" ? "text" : "number"} min="0" value={rule.value} onChange={e => updateRule(index, { value: rule.field === "tag" ? e.target.value : Number(e.target.value) })} placeholder={rule.field === "tag" ? "VIP" : "0"} />
              <button className="secondary-button" disabled={rules.length === 1} onClick={() => setRules(current => current.filter((_, i) => i !== index))}>Remove</button>
            </div>
          ))}
          <div>
            <button className="secondary-button" onClick={() => setRules(current => [...current, emptyRule()])} disabled={rules.length >= 10}>Add rule</button>{" "}
            <button className="primary-button" onClick={() => void create()} disabled={saving || !name.trim()}>{saving ? "Creating…" : "Create segment"}</button>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="detail-header">
          <div>
            ...
            <p className="muted">Counts are evaluated from current customer and order data.</p>
          </div>
        </div>
        {loading ? (
          <p className="muted">Loading segments...</p>
        ) : segments.length === 0 ? (
          <p className="muted">No segments yet.</p>
        ) : (
          segments.map(segment => (
            <div className="segment-card" key={segment.id}>
              <div onClick={() => void open(segment.id)} className="segment-main">
                <strong>{segment.name}</strong>
                <span className="muted">{segment.customerCount} customers · {segment.rules.rules.length} rule{segment.rules.rules.length === 1 ? "" : "s"}</span>
                <small className="muted">{segment.description || "No description"}</small>
              </div>
              <button className="secondary-button" onClick={() => void remove(segment.id)}>Delete</button>
            </div>
          ))
        )}
      </section>
      ...
      {selectedId && (
        <section className="section">
          <h2>Segment members</h2>
          {selected.length === 0 ? (
            <p className="muted">No customers currently match this segment.</p>
          ) : (
            <>
              <div className="table-wrap">
                <table>
                  <thead>
                    <tr><th>Customer</th><th>Contact</th><th>Orders</th><th>Spend</th><th>Last order</th><th>Tags</th></tr>
                  </thead>
                  <tbody>
                    {selected.map(member => (
                      <tr key={member.id}>
                        <td><strong>{[member.firstName, member.lastName].filter(Boolean).join(" ") || "Unnamed customer"}</strong></td>
                        <td>{member.email || member.phone || "—"}</td>
                        <td>{member.orderCount}</td>
                        <td>PKR {member.totalSpend}</td>
                        <td>{member.lastOrderAt ? new Date(member.lastOrderAt).toLocaleDateString() : "—"}</td>
              ...
              {memberTotalPages > 1 && (
                <div className="detail-header">
                  <span className="muted">Page {memberPage} of {memberTotalPages}</span>
                  <div className="tag-list">
                    <button className="secondary-button" onClick={() => void open(selectedId, memberPage - 1)} disabled={memberPage === 1}>Previous</button>
                    <button className="secondary-button" onClick={() => void open(selectedId, memberPage + 1)} disabled={memberPage === memberTotalPages}>Next</button>
                  </div>
                </div>
              )}
            </>
