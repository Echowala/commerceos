...
export default function SegmentsPage() {
  ...
  const open = async (id: string) => { setSelectedId(id); try { setSelected(await api<Member[]>(`/crm/segments/${id}/customers`)); } catch (e) { setError(e instanceof Error ? e.message : "Unable to load segment members"); } };
  const remove = async (id: string) => { if (!window.confirm("Delete this segment?")) return; try { await api(`/crm/segments/${id}`, { method: "DELETE" }); if (selectedId === id) { setSelectedId(""); setSelected([]); } await load(); } catch (e) { setError(e instanceof Error ? e.message : "Unable to delete segment"); } };
