...
export default function CustomersPage() {
  ...
  const load = async () => { try { setCustomers(await api<Customer[]>("/customers")); setError(""); } catch (e) { setError(e instanceof Error ? e.message : "Unable to load customers"); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, []);
  const openCustomer = async (id: string) => { try { setSelected(await api<CustomerDetail>(`/customers/${id}`)); setError(""); } catch (e) { setError(e instanceof Error ? e.message : "Unable to load customer"); } };

  return <main className="main workspace-page">
    <header className="header"><div><h1 className="title">Customers</h1><div className="muted">Customer 360: identity, orders, spend and buying patterns.</div></div><a className="back-link" href="/">Dashboard</a></header>
